...
class Drag extends Base {
  ...
  createCloneNode() {
    if (!this.clone) {
      ...
      if (this.beingDragNodeList.length > 1) {
        ...
        group.add(numNode)
        this.clone = t.group.clone()
        this.clone.add(group)
  ...
  handleVerticalCheck(node, checkList, isReverse = false) {
    ...
    let threeFourthHeight = 3 * nodeRect.height / 4
      ...
      if (
        ...
        let checkIsPrevNode =
          nextBrotherOffset >= 0 // 距离下一个兄弟节点的距离大于0
            ? y - this.offsetY > nodeRect.bottom && y - this.offsetY <= nodeRect.bottom + threeFourthHeight // 那么在当前节点外底部判断
            : y - this.offsetY >= nodeRect.bottom - threeFourthHeight && y - this.offsetY <= nodeRect.bottom // 否则在当前节点内底部3/4区间判断
        let checkIsNextNode =
          prevBrotherOffset >= 0 // 距离上一个兄弟节点的距离大于0
            ? y - this.offsetY + this.mousedownNode.height < nodeRect.top && y - this.offsetY + this.mousedownNode.height >= nodeRect.top - threeFourthHeight // 那么在当前节点外底部判断
            : y - this.offsetY + this.mousedownNode.height >= nodeRect.top && y - this.offsetY + this.mousedownNode.height <= nodeRect.top + threeFourthHeight
        if (checkIsPrevNode) {
          if (isReverse) {
            this.nextNode = node
          } else {
          ...
          let size = this.formatPlaceholderSize(threeFourthHeight)
  ...
  handleHorizontalCheck(node, checkList) {
    ...
    let y = this.mouseMoveY
    let nodeRect = this.getNodeRect(node)
    let threeFourthWidth = 3 * nodeRect.width / 4
    let { prevBrotherOffset, nextBrotherOffset } =
      this.getNodeDistanceToSiblingNode(checkList, node, nodeRect, 'h')
    if (nodeRect.top <= y - this.offsetY + this.mousedownNode.height && nodeRect.bottom >= y - this.offsetY) {
      // 检测兄弟节点位置
      if (
        !this.overlapNode &&
        !this.prevNode &&
        ...
        let checkIsPrevNode =
          nextBrotherOffset > 0 // 距离下一个兄弟节点的距离大于0
            ? x - this.offsetX < nodeRect.right + threeFourthWidth && x - this.offsetX >= nodeRect.right // 那么在当前节点外底部判断
            : x - this.offsetX <= nodeRect.right && x - this.offsetX >= nodeRect.right - threeFourthWidth // 否则在当前节点内底部3/4区间判断
        let checkIsNextNode =
          prevBrotherOffset > 0 // 距离上一个兄弟节点的距离大于0
            ? x - this.offsetX + this.mousedownNode.width > nodeRect.left - threeFourthWidth && x - this.offsetX + this.mousedownNode.width <= nodeRect.left // 那么在当前节点外底部判断
            : x - this.offsetX + this.mousedownNode.width <= nodeRect.left + threeFourthWidth && x - this.offsetX + this.mousedownNode.width >= nodeRect.left
        if (checkIsPrevNode) {
          this.prevNode = node
          let size = this.formatPlaceholderSize(threeFourthWidth)
          this.setPlaceholderRect(
            size,
            node.height,
  ...
  checkIsOverlap({
    ...
    let dir1 = dir === 'v' ? 'top' : 'left'
    let dir2 = dir === 'v' ? 'bottom' : 'right'
    let offset = dir === 'v' ? this.offsetY : this.offsetX, len = dir === 'v' ? this.mousedownNode.height : this.mousedownNode.width
    if (!this.overlapNode && !this.prevNode && !this.nextNode) {
      if (
        nodeRect[dir1] + (prevBrotherOffset > 0 ? 0 : size) < pos - offset + len &&
        nodeRect[dir2] - (nextBrotherOffset > 0 ? 0 : size) > pos - offset
      ) {
